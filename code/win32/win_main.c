...
char **Sys_ListFiles( const char *directory, const char *extension, char *filter, int *numfiles, qboolean wantsubs ) {
	...
	if (filter) {
		...
		*numfiles = nfiles;
		...
		listCopy = (char **)Z_Malloc( ( nfiles + 1 ) * sizeof( *listCopy ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		for ( i = 0 ; i < nfiles ; i++ ) {
			listCopy[i] = list[i];
		}
		listCopy[i] = NULL;
...
char *Sys_GetClipboardData( void ) {
	...
	if ( OpenClipboard( NULL ) != 0 ) {
		...
		if ( ( hClipboardData = GetClipboardData( CF_TEXT ) ) != 0 ) {
			if ( ( cliptext = (char *)GlobalLock( hClipboardData ) ) != 0 ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast
				data = (char *)Z_Malloc( GlobalSize( hClipboardData ) + 1 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void Sys_UnloadDll( void *dllHandle ) {
	...
	if ( !FreeLibrary( (HMODULE)dllHandle ) ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
sysEvent_t Sys_GetEvent( void ) {
	...
	if ( s ) {
		...
		len = strlen( s ) + 1;
		b = (char *)Z_Malloc( len ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	if ( Sys_GetPacket ( &adr, &netmsg ) ) {
		...
		buf = (netadr_t *)Z_Malloc( len ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		*buf = adr;
