...
qboolean IN_InitDIMouse( void ) {
	...
	if (!pDirectInputCreate) {
		...
		pDirectInputCreate = (HRESULT (__stdcall *)(HINSTANCE,DWORD,LPDIRECTINPUT *,LPUNKNOWN))GetProcAddress(hInstDI,"DirectInputCreateA"); // ***GREGS_VC9_PORT_MOD*** -- needed to change function pointer typecast (the original line is above)
	...
	hr = IDirectInput_CreateDevice(g_pdi, /*&*/GUID_SysMouse, &g_pMouse, NULL); // ***GREGS_VC9_PORT_MOD*** -- took out "&" when referring to interfaces and class id's
