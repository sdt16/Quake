...
static qboolean GLW_InitDriver( const char *drivername, int colorbits )
{
	...
	if ( !glw_state.pixelFormatSet )
	{
		GLW_CreatePFD( &pfd, colorbits, depthbits, stencilbits, (qboolean)r_stereo->integer );  // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		if ( ( tpfd = GLW_MakeContext( &pfd ) ) != TRY_PFD_SUCCESS )
		{
			...
			GLW_CreatePFD( &pfd, colorbits, depthbits, 0, (qboolean)r_stereo->integer );  // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static qboolean GLW_CreateWindow( const char *drivername, int width, int height, int colorbits, qboolean cdsFullscreen )
{
	...
	if ( !s_classRegistered )
	{
		...
		wc.hbrBackground = (HBRUSH)COLOR_GRAYTEXT; // ***GREGS_VC9_PORT_MOD*** -- changed typecast from void * to HBRUSH
		wc.lpszMenuName  = 0;
...
static qboolean GLW_LoadOpenGL( const char *drivername )
{
	...
	if ( QGL_Init( buffer ) ) 
	{
		cdsFullscreen = (qboolean)r_fullscreen->integer; // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void GLimp_Init( void )
{
	...
	Q_strncpyz( glConfig.vendor_string, (const char *)qglGetString (GL_VENDOR), sizeof( glConfig.vendor_string ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	Q_strncpyz( glConfig.renderer_string, (const char *)qglGetString (GL_RENDERER), sizeof( glConfig.renderer_string ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	Q_strncpyz( glConfig.version_string, (const char *)qglGetString (GL_VERSION), sizeof( glConfig.version_string ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	Q_strncpyz( glConfig.extensions_string, (const char *)qglGetString (GL_EXTENSIONS), sizeof( glConfig.extensions_string ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
qboolean GLimp_SpawnRenderThread( void (*function)( void ) ) {
	...
	renderThreadHandle = CreateThread(
	   ...
	   0,			//   DWORD fdwCreate,
	   (LPDWORD)&renderThreadId ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
