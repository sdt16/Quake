...
extern cvar_t		*r_fullscreen; // ***GREGS_VC9_PORT_MOD*** -- added extern: r_fullscreen already exists in tr_init (renderer project)
...
{
	g_wv.isMinimized = (qboolean)minimize; // ***GREGS_VC9_PORT_MOD*** -- needed typecast
			...
			if ( zDelta > 0 )
			{
				for(i=0; i<zDelta; i++)
				{
					...
					else
					{
						...
						flip = (qboolean)(!flip); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
					}
				}
			}
			else
