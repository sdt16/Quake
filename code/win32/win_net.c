...
Modified by Greg Dolley [12/29/2007]
===========================================================================
*/
// net_wins.c
		...
		if( s[0] >= '0' && s[0] <= '9' ) {
			*(int *)&((struct sockaddr_in *)sadr)->sin_addr = inet_addr(s);
			...
			if( ( h = gethostbyname( s ) ) == 0 ) {
				return((qboolean)0); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
			}
...
qboolean Sys_GetPacket( netadr_t *net_from, msg_t *net_message ) {
	...
	for( protocol = 0 ; protocol < 2 ; protocol++ )	{
		...
		ret = recvfrom( net_socket, (char *)(net_message->data), net_message->maxsize, 0, (struct sockaddr *)&from, &fromlen ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
		if (ret == SOCKET_ERROR)
		{
			err = WSAGetLastError();
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if( usingSocks && to.type == NA_IP ) {
		...
		*(short *)&socksBuf[8] = ((struct sockaddr_in *)&addr)->sin_port;
		...
		ret = sendto( net_socket, (const char *)data, length, 0, &addr, sizeof(addr) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
	}
	if( ret == SOCKET_ERROR ) {
...
int NET_IPSocket( char *net_interface, int port ) {
	...
	if( ioctlsocket( newsocket, FIONBIO, (u_long *)&_true ) == SOCKET_ERROR ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	if( bind( newsocket, (const sockaddr *)&address, sizeof(address) ) == SOCKET_ERROR ) {  // ***GREGS_VC9_PORT_MOD*** -- changed typecast; was void *
...
void NET_OpenSocks( int port ) {
	...
	if ( send( socks_socket, (const char *)buf, len, 0 ) == SOCKET_ERROR ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
		err = WSAGetLastError();
	...
	// get the response
	len = recv( socks_socket, (char *)buf, 64, 0 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
	if ( len == SOCKET_ERROR ) {
		err = WSAGetLastError();
	...
	if ( buf[1] == 2 ) {
		...
		// send it
		if ( send( socks_socket, (const char *)buf, 3 + ulen + plen, 0 ) == SOCKET_ERROR ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
			err = WSAGetLastError();
			Com_Printf( "NET_OpenSocks: send: %s\n", NET_ErrorString() );
			return;
		}
	...
	*(short *)&buf[8] = htons( (short)port );		// port
	if ( send( socks_socket, (const char *)buf, 10, 0 ) == SOCKET_ERROR ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast on second parameter
		err = WSAGetLastError();
		Com_Printf( "NET_OpenSocks: send: %s\n", NET_ErrorString() );
		return;
	}
...
int NET_IPXSocket( int port ) {
	...
	// make it non-blocking
	if( ioctlsocket( newsocket, FIONBIO, (u_long *)(&_true) ) == SOCKET_ERROR ) { // ***GREGS_VC9_PORT_MOD*** -- needed typecast on third parameter
