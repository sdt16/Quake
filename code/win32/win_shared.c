...
#ifdef _MANAGED
	// ***GREGS_VC9_PORT_MOD*** -- the CPUID() function uses assembly language which is incompatible
	//                             with the .NET Framework (corrupts stack frame and bad crash);
	//                             since returning false from this function results in the rest of
	//                             the code evaluating the CPU as a generic Pentium we are OK; however,
	//                             it's probably best to remove this CPU checking entirely some day in
	//                             the near future because it's practically irrelevant with the kind of
