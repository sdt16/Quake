...
/*qboolean*/ int	anykeydown; // ***GREGS_VC9_PORT_MOD*** -- changed from qboolean to int because the variable was being used like an int when it was qboolean (totally illegal in C++)
...
void Field_KeyDownEvent( field_t *edit, int key ) {
	...
	if ( key == K_INS ) {
		key_overstrikeMode = (qboolean)(!key_overstrikeMode); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		return;
	}
}
...
void Key_ClearStates (void)
{
	...
	for ( i=0 ; i < MAX_KEYS ; i++ ) {
		...
		keys[i].down = (qboolean)0; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		keys[i].repeats = 0;
	}
}
