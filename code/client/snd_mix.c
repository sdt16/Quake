...
//Com_Printf ("%i to %i\n", s_paintedtime, endtime);
	while ( s_paintedtime < endtime ) {
		...
		if ( s_rawend < s_paintedtime ) {
			...
			// ***GREGS_VC9_PORT_MOD*** -- FIXME - Figure out why s_paintedtime is usually below zero in this .NET port.
			// When the "if(s_paintedtime < 0)" check is here, it prevents the crash (access violation indexing paintbuffer[]). 
			// I haven't noticed anything else being affected. The sound still works fine. So the priority on fixing this bug 
			// is pretty low.
			//
			// I suspect it has something to do with this module's ASM code not playing nice with .NET. I've seen this happen
			// before with the CPU detection ASM - that ASM code worked fine after the native C++ port, but then stopped working 
			// under MC++ .NET.
			// [Greg Dolley]
			if(s_paintedtime < 0) // ***GREGS_VC9_PORT_MOD*** -- I added this if(), not part of original codebase (see above) 
				return;
