...
redump:
	switch(cinTable[currentHandle].roq_id) 
	{
		...
		case	ROQ_QUAD_INFO:
			...
			break;
		case	ROQ_PACKET:
			cinTable[currentHandle].inMemory = (qboolean)(cinTable[currentHandle].roq_flags); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
//
	...
	{ 
		/*cinTable[currentHandle].inMemory--*/ /* TODO: if "inMemory = qfalse" below works, then delete this commented inMemory block. [Greg Dolley]*/ 
		cinTable[currentHandle].inMemory = qfalse; framedata += 8; goto redump;
	}
//
...
e_status CIN_RunCinematic (int handle)
{
	...
	if (cinTable[currentHandle].shader && (abs((int)(thisTime - cinTable[currentHandle].lastTime)))>100) { // ***GREGS_VC9_PORT_MOD*** -- added int typecast in abs() param
...
int CIN_PlayCinematic( const char *arg, int x, int y, int w, int h, int systemBits ) {
	...
	CIN_SetLooping(currentHandle, (qboolean)((systemBits & CIN_loop)!=0)); // ***GREGS_VC9_PORT_MOD*** -- needed typecast

	cinTable[currentHandle].CIN_HEIGHT = DEFAULT_CIN_HEIGHT;
	cinTable[currentHandle].CIN_WIDTH  =  DEFAULT_CIN_WIDTH;
	cinTable[currentHandle].holdAtEnd = (qboolean)((systemBits & CIN_hold) != 0); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	cinTable[currentHandle].alterGameState = (qboolean)((systemBits & CIN_system) != 0); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	cinTable[currentHandle].playonwalls = 1;
	cinTable[currentHandle].silent = (qboolean)((systemBits & CIN_silent) != 0); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	cinTable[currentHandle].shader = (qboolean)((systemBits & CIN_shader) != 0); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void CIN_DrawCinematic (int handle) {
	...
	if (cinTable[handle].dirty && (cinTable[handle].CIN_WIDTH != cinTable[handle].drawX || cinTable[handle].CIN_HEIGHT != cinTable[handle].drawY)) {
		...
		buf3 = (int*)buf;
		buf2 = (int *)Hunk_AllocateTempMemory( 256*256*4 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
                if (xm==2 && ym==2) {
                    byte *bc2, *bc3;
                    int	ic, iiy;
