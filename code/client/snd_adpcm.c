...
void S_AdpcmGetSamples(sndBuffer *chunk, short *to) {
	...
	// get samples
	S_AdpcmDecode( (const char *)out, to, SND_CHUNK_SIZE_BYTE*2, &state ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
void S_AdpcmEncodeSound( sfx_t *sfx, short *samples ) {
	...
	while( count ) {
		...
		// encode the samples
		S_AdpcmEncode( samples + inOffset, (char *)out, n, &state ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
