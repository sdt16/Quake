...
#pragma warning (disable:4389) // ***GREGS_VC9_PORT_MOD*** -- signed/unsigned mismatch; I don't know why this warning occurs in Release mode, but not in Debug
...
static void R_MipMap2( unsigned *in, int inWidth, int inHeight ) {
	...
	temp = (unsigned int *)ri.Hunk_AllocateTempMemory( outWidth * outHeight * 4 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	if ( scaled_width != width || scaled_height != height ) {
		resampledBuffer = (unsigned int *)ri.Hunk_AllocateTempMemory( scaled_width * scaled_height * 4 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	scaledBuffer = (unsigned int *)ri.Hunk_AllocateTempMemory( sizeof( unsigned ) * scaled_width * scaled_height ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	R_LightScaleTexture (scaledBuffer, scaled_width, scaled_height, (qboolean)(!mipmap) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
image_t *R_CreateImage( const char *name, const byte *pic, int width, int height, 
	...
	image = tr.images[tr.numImages] = (image_t *)ri.Hunk_Alloc( sizeof( image_t ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static void LoadBMP( const char *name, byte **pic, int *width, int *height )
{
	...
	*pic = NULL;
	...
	bmpRGBA = (byte *)ri.Malloc( numPixels * 4 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	*pic = bmpRGBA;
...
static void LoadPCX ( const char *filename, byte **pic, byte **palette, int *width, int *height)
{
	...
	*palette = NULL;
	...
	out = (byte *)ri.Malloc ( (ymax+1) * (xmax+1) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	*pic = out;
	...
	if (palette)
	{
		*palette = (byte *)ri.Malloc(768); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static void LoadPCX32 ( const char *filename, byte **pic, int *width, int *height) {
	...
	c = (*width) * (*height);
	pic32 = *pic = (byte *)ri.Malloc(4 * c ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	for (i = 0 ; i < c ; i++) {
		p = pic8[i];
		pic32[0] = palette[p*3];
...
static void LoadTGA ( const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	targa_rgba = (byte *)ri.Malloc (numPixels*4); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	*pic = targa_rgba;
...
static void LoadJPG( const char *filename, unsigned char **pic, int *width, int *height ) {
  ...
  out = (unsigned char *)ri.Malloc(cinfo.output_width*cinfo.output_height*cinfo.output_components); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void SaveJPG(char * filename, int quality, int image_width, int image_height, unsigned char *image_buffer) {
   ...
   */
  out = (unsigned char *)ri.Hunk_AllocateTempMemory(image_width*image_height*4); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static void R_CreateFogImage( void ) {
	...
	data = (byte *)ri.Hunk_AllocateTempMemory( FOG_S * FOG_T * 4 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	tr.numSkins++;
	skin = (skin_t *)ri.Hunk_Alloc( sizeof( skin_t ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	tr.skins[hSkin] = skin;
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		skin->numSurfaces = 1;
		skin->surfaces[0] = (skinSurface_t *)ri.Hunk_Alloc( sizeof(skin->surfaces[0]), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	while ( text_p && *text_p ) {
		...
		surf = skin->surfaces[ skin->numSurfaces ] = (skinSurface_t *)ri.Hunk_Alloc( sizeof( *skin->surfaces[0] ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void	R_InitSkins( void ) {
	...
	skin = tr.skins[0] = (skin_t *)ri.Hunk_Alloc( sizeof( skin_t ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	Q_strncpyz( skin->name, "<default skin>", sizeof( skin->name )  );
	skin->numSurfaces = 1;
	skin->surfaces[0] = (skinSurface_t *)ri.Hunk_Alloc( sizeof( *skin->surfaces ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
