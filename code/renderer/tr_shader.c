...
static qboolean ParseStage( shaderStage_t *stage, char **text )
{
	...
	while ( 1 )
	{
		...
		else if ( !Q_stricmp( token, "map" ) )
		{
			...
			{
				stage->bundle[0].image[0] = R_FindImageFile( token, (qboolean)(!shader.noMipMaps), (qboolean)(!shader.noPicMip), GL_REPEAT ); // ***GREGS_VC9_PORT_MOD*** -- needed typecasts
		...
		else if ( !Q_stricmp( token, "clampmap" ) )
		{
			...
			stage->bundle[0].image[0] = R_FindImageFile( token, (qboolean)(!shader.noMipMaps), (qboolean)(!shader.noPicMip), GL_CLAMP ); // ***GREGS_VC9_PORT_MOD*** -- needed typecasts
			if ( !stage->bundle[0].image[0] )
			{
				ri.Printf( PRINT_WARNING, "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name );
				return qfalse;
		...
		else if ( !Q_stricmp( token, "animMap" ) )
		{
			...
			while ( 1 ) {
				...
				if ( num < MAX_IMAGE_ANIMATIONS ) {
					stage->bundle[0].image[num] = R_FindImageFile( token, (qboolean)(!shader.noMipMaps), (qboolean)(!shader.noPicMip), GL_REPEAT ); // ***GREGS_VC9_PORT_MOD*** -- needed typecasts
...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = (shader_t *)ri.Hunk_Alloc( sizeof( shader_t ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = (shaderStage_t *)ri.Hunk_Alloc( sizeof( stages[i] ), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			newShader->stages[i]->bundle[b].texMods = (texModInfo_t *)ri.Hunk_Alloc( size, h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static void ScanAndLoadShaderFiles( void )
{
	...
	s_shaderText = (char *)ri.Hunk_Alloc( sum + numShaders*2, h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	hashMem = (char *)ri.Hunk_Alloc( size * sizeof(char *), h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
