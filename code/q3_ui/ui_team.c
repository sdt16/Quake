...
void TeamMain_MenuInit( void ) {
	...
	s_teammain.frame.generic_mc.type   = MTYPE_BITMAP;
	s_teammain.frame.generic_mc.flags	= QMF_INACTIVE;
	s_teammain.frame.generic_mc.name   = TEAMMAIN_FRAME;
	s_teammain.frame.generic_mc.x		= 142;
	s_teammain.frame.generic_mc.y		= 118;
	s_teammain.frame.width			= 359;
	s_teammain.frame.height			= 256;

	y = 194;

	s_teammain.joinred.generic_mc.type     = MTYPE_PTEXT;
	s_teammain.joinred.generic_mc.flags    = QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_teammain.joinred.generic_mc.id       = ID_JOINRED;
	s_teammain.joinred.generic_mc.callback = TeamMain_MenuEvent;
	s_teammain.joinred.generic_mc.x        = 320;
	s_teammain.joinred.generic_mc.y        = y;
	s_teammain.joinred.string           = "JOIN RED";
	s_teammain.joinred.style            = UI_CENTER|UI_SMALLFONT;
	s_teammain.joinred.color            = colorRed;
	y += 20;

	s_teammain.joinblue.generic_mc.type     = MTYPE_PTEXT;
	s_teammain.joinblue.generic_mc.flags    = QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_teammain.joinblue.generic_mc.id       = ID_JOINBLUE;
	s_teammain.joinblue.generic_mc.callback = TeamMain_MenuEvent;
	s_teammain.joinblue.generic_mc.x        = 320;
	s_teammain.joinblue.generic_mc.y        = y;
	s_teammain.joinblue.string           = "JOIN BLUE";
	s_teammain.joinblue.style            = UI_CENTER|UI_SMALLFONT;
	s_teammain.joinblue.color            = colorRed;
	y += 20;

	s_teammain.joingame.generic_mc.type     = MTYPE_PTEXT;
	s_teammain.joingame.generic_mc.flags    = QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_teammain.joingame.generic_mc.id       = ID_JOINGAME;
	s_teammain.joingame.generic_mc.callback = TeamMain_MenuEvent;
	s_teammain.joingame.generic_mc.x        = 320;
	s_teammain.joingame.generic_mc.y        = y;
	s_teammain.joingame.string           = "JOIN GAME";
	s_teammain.joingame.style            = UI_CENTER|UI_SMALLFONT;
	s_teammain.joingame.color            = colorRed;
	y += 20;

	s_teammain.spectate.generic_mc.type     = MTYPE_PTEXT;
	s_teammain.spectate.generic_mc.flags    = QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_teammain.spectate.generic_mc.id       = ID_SPECTATE;
	s_teammain.spectate.generic_mc.callback = TeamMain_MenuEvent;
	s_teammain.spectate.generic_mc.x        = 320;
	s_teammain.spectate.generic_mc.y        = y;
	...
	case GT_FFA:
	case GT_TOURNAMENT:
		s_teammain.joinred.generic_mc.flags  |= QMF_GRAYED;
		s_teammain.joinblue.generic_mc.flags |= QMF_GRAYED;
		break;

	default:
	case GT_TEAM:
	case GT_CTF:
		s_teammain.joingame.generic_mc.flags |= QMF_GRAYED;
		break;
	}
