...
static void PlayerModel_UpdateGrid( void )
{
	...
	for (i=0; i<PLAYERGRID_ROWS*PLAYERGRID_COLS; i++,j++)
	{
		...
		{ 
			// model/skin portrait
 			s_playermodel.pics[i].generic_mc.name         = s_playermodel.modelnames[j];
			s_playermodel.picbuttons[i].generic_mc.flags &= ~QMF_INACTIVE;
		}
		else
		{
			// dead slot
 			s_playermodel.pics[i].generic_mc.name         = NULL;
			s_playermodel.picbuttons[i].generic_mc.flags |= QMF_INACTIVE;
		}

 		s_playermodel.pics[i].generic_mc.flags       &= ~QMF_HIGHLIGHT;
 		s_playermodel.pics[i].shader               = 0;
 		s_playermodel.picbuttons[i].generic_mc.flags |= QMF_PULSEIFFOCUS;
	}
	...
	if (s_playermodel.numpages > 1)
	{
		if (s_playermodel.modelpage > 0)
			s_playermodel.left.generic_mc.flags &= ~QMF_INACTIVE;
		else
			s_playermodel.left.generic_mc.flags |= QMF_INACTIVE;

		if (s_playermodel.modelpage < s_playermodel.numpages-1)
			s_playermodel.right.generic_mc.flags &= ~QMF_INACTIVE;
		else
			s_playermodel.right.generic_mc.flags |= QMF_INACTIVE;
	}
	else
...
static sfxHandle_t PlayerModel_MenuKey( int key )
{
	...
	switch (key)
	{
		case K_KP_LEFTARROW:
		case K_LEFTARROW:
			m = (menucommon_s *)Menu_ItemAtCursor(&s_playermodel.menu); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
static void PlayerModel_DrawPlayer( void *self )
{
	...
	if( trap_MemoryRemaining() <= LOW_MEMORY ) {
		UI_DrawProportionalString( b->generic_mc.x, b->generic_mc.y + b->height / 2, "LOW MEMORY", UI_LEFT, color_red );
		return;
	}

	UI_DrawPlayer( b->generic_mc.x, b->generic_mc.y, b->width, b->height, &s_playermodel.playerinfo, uis.realtime/2 );
}
...
static void PlayerModel_MenuInit( void )
{
	...
	s_playermodel.banner.generic_mc.type  = MTYPE_BTEXT;
	s_playermodel.banner.generic_mc.x     = 320;
	s_playermodel.banner.generic_mc.y     = 16;
	s_playermodel.banner.string        = "PLAYER MODEL";
	s_playermodel.banner.color         = color_white;
	s_playermodel.banner.style         = UI_CENTER;

	s_playermodel.framel.generic_mc.type  = MTYPE_BITMAP;
	s_playermodel.framel.generic_mc.name  = MODEL_FRAMEL;
	s_playermodel.framel.generic_mc.flags = QMF_LEFT_JUSTIFY|QMF_INACTIVE;
	s_playermodel.framel.generic_mc.x     = 0;
	s_playermodel.framel.generic_mc.y     = 78;
	s_playermodel.framel.width         = 256;
	s_playermodel.framel.height        = 329;

	s_playermodel.framer.generic_mc.type  = MTYPE_BITMAP;
	s_playermodel.framer.generic_mc.name  = MODEL_FRAMER;
	s_playermodel.framer.generic_mc.flags = QMF_LEFT_JUSTIFY|QMF_INACTIVE;
	s_playermodel.framer.generic_mc.x     = 376;
	s_playermodel.framer.generic_mc.y     = 76;
	s_playermodel.framer.width         = 256;
	s_playermodel.framer.height        = 334;

	s_playermodel.ports.generic_mc.type  = MTYPE_BITMAP;
	s_playermodel.ports.generic_mc.name  = MODEL_PORTS;
	s_playermodel.ports.generic_mc.flags = QMF_LEFT_JUSTIFY|QMF_INACTIVE;
	s_playermodel.ports.generic_mc.x     = 50;
	s_playermodel.ports.generic_mc.y     = 59;
	...
	for (i=0,k=0; i<PLAYERGRID_ROWS; i++)
	{
		...
		{
			s_playermodel.pics[k].generic_mc.type	   = MTYPE_BITMAP;
			s_playermodel.pics[k].generic_mc.flags    = QMF_LEFT_JUSTIFY|QMF_INACTIVE;
			s_playermodel.pics[k].generic_mc.x		   = x;
			s_playermodel.pics[k].generic_mc.y		   = y;
			...
			s_playermodel.picbuttons[k].generic_mc.type	 = MTYPE_BITMAP;
			s_playermodel.picbuttons[k].generic_mc.flags    = QMF_LEFT_JUSTIFY|QMF_NODEFAULTINIT|QMF_PULSEIFFOCUS;
			s_playermodel.picbuttons[k].generic_mc.id	     = ID_PLAYERPIC0+k;
			s_playermodel.picbuttons[k].generic_mc.callback = PlayerModel_PicEvent;
			s_playermodel.picbuttons[k].generic_mc.x    	 = x - 16;
			s_playermodel.picbuttons[k].generic_mc.y		 = y - 16;
			s_playermodel.picbuttons[k].generic_mc.left	 = x;
			s_playermodel.picbuttons[k].generic_mc.top		 = y;
			s_playermodel.picbuttons[k].generic_mc.right	 = x + 64;
			s_playermodel.picbuttons[k].generic_mc.bottom   = y + 64;
	...
	s_playermodel.playername.generic_mc.type  = MTYPE_PTEXT;
	s_playermodel.playername.generic_mc.flags = QMF_CENTER_JUSTIFY|QMF_INACTIVE;
	s_playermodel.playername.generic_mc.x	   = 320;
	s_playermodel.playername.generic_mc.y	   = 440;
	...
	s_playermodel.modelname.generic_mc.type  = MTYPE_PTEXT;
	s_playermodel.modelname.generic_mc.flags = QMF_CENTER_JUSTIFY|QMF_INACTIVE;
	s_playermodel.modelname.generic_mc.x	  = 497;
	s_playermodel.modelname.generic_mc.y	  = 54;
	...
	s_playermodel.skinname.generic_mc.type   = MTYPE_PTEXT;
	s_playermodel.skinname.generic_mc.flags  = QMF_CENTER_JUSTIFY|QMF_INACTIVE;
	s_playermodel.skinname.generic_mc.x	  = 497;
	s_playermodel.skinname.generic_mc.y	  = 394;
	...
	s_playermodel.player.generic_mc.type      = MTYPE_BITMAP;
	s_playermodel.player.generic_mc.flags     = QMF_INACTIVE;
	s_playermodel.player.generic_mc.ownerdraw = PlayerModel_DrawPlayer;
	s_playermodel.player.generic_mc.x	       = 400;
	s_playermodel.player.generic_mc.y	       = -40;
	s_playermodel.player.width	           = 32*10;
	s_playermodel.player.height            = 56*10;

	s_playermodel.arrows.generic_mc.type		= MTYPE_BITMAP;
	s_playermodel.arrows.generic_mc.name		= MODEL_ARROWS;
	s_playermodel.arrows.generic_mc.flags		= QMF_INACTIVE;
	s_playermodel.arrows.generic_mc.x			= 125;
	s_playermodel.arrows.generic_mc.y			= 340;
	s_playermodel.arrows.width				= 128;
	s_playermodel.arrows.height				= 32;

	s_playermodel.left.generic_mc.type			= MTYPE_BITMAP;
	s_playermodel.left.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_playermodel.left.generic_mc.callback		= PlayerModel_MenuEvent;
	s_playermodel.left.generic_mc.id			= ID_PREVPAGE;
	s_playermodel.left.generic_mc.x			= 125;
	s_playermodel.left.generic_mc.y			= 340;
	s_playermodel.left.width  				= 64;
	s_playermodel.left.height  				= 32;
	s_playermodel.left.focuspic				= MODEL_ARROWSL;

	s_playermodel.right.generic_mc.type	    = MTYPE_BITMAP;
	s_playermodel.right.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_playermodel.right.generic_mc.callback	= PlayerModel_MenuEvent;
	s_playermodel.right.generic_mc.id			= ID_NEXTPAGE;
	s_playermodel.right.generic_mc.x			= 125+61;
	s_playermodel.right.generic_mc.y			= 340;
	s_playermodel.right.width  				= 64;
	s_playermodel.right.height  		    = 32;
	s_playermodel.right.focuspic			= MODEL_ARROWSR;

	s_playermodel.back.generic_mc.type	    = MTYPE_BITMAP;
	s_playermodel.back.generic_mc.name     = MODEL_BACK0;
	s_playermodel.back.generic_mc.flags    = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_playermodel.back.generic_mc.callback = PlayerModel_MenuEvent;
	s_playermodel.back.generic_mc.id	    = ID_BACK;
	s_playermodel.back.generic_mc.x		= 0;
	s_playermodel.back.generic_mc.y		= 480-64;
