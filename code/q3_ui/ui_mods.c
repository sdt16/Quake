...
static void UI_Mods_MenuInit( void ) {
	...
	s_mods.banner.generic_mc.type		= MTYPE_BTEXT;
	s_mods.banner.generic_mc.x			= 320;
	s_mods.banner.generic_mc.y			= 16;
	s_mods.banner.string			= "MODS";
	s_mods.banner.color				= color_white;
	s_mods.banner.style				= UI_CENTER;

	s_mods.framel.generic_mc.type		= MTYPE_BITMAP;
	s_mods.framel.generic_mc.name		= ART_FRAMEL;
	s_mods.framel.generic_mc.flags		= QMF_INACTIVE;
	s_mods.framel.generic_mc.x			= 0;  
	s_mods.framel.generic_mc.y			= 78;
	s_mods.framel.width				= 256;
	s_mods.framel.height			= 329;

	s_mods.framer.generic_mc.type		= MTYPE_BITMAP;
	s_mods.framer.generic_mc.name		= ART_FRAMER;
	s_mods.framer.generic_mc.flags		= QMF_INACTIVE;
	s_mods.framer.generic_mc.x			= 376;
	s_mods.framer.generic_mc.y			= 76;
	s_mods.framer.width				= 256;
	s_mods.framer.height			= 334;

	s_mods.back.generic_mc.type		= MTYPE_BITMAP;
	s_mods.back.generic_mc.name		= ART_BACK0;
	s_mods.back.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_mods.back.generic_mc.id			= ID_BACK;
	s_mods.back.generic_mc.callback	= UI_Mods_MenuEvent;
	s_mods.back.generic_mc.x			= 0;
	s_mods.back.generic_mc.y			= 480-64;
	s_mods.back.width				= 128;
	s_mods.back.height				= 64;
	s_mods.back.focuspic			= ART_BACK1;

	s_mods.go.generic_mc.type			= MTYPE_BITMAP;
	s_mods.go.generic_mc.name			= ART_FIGHT0;
	s_mods.go.generic_mc.flags			= QMF_RIGHT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_mods.go.generic_mc.id			= ID_GO;
	s_mods.go.generic_mc.callback		= UI_Mods_MenuEvent;
	s_mods.go.generic_mc.x				= 640;
	s_mods.go.generic_mc.y				= 480-64;
	s_mods.go.width					= 128;
	s_mods.go.height				= 64;
	s_mods.go.focuspic				= ART_FIGHT1;

	// scan for mods
	s_mods.list.generic_mc.type		= MTYPE_SCROLLLIST;
	s_mods.list.generic_mc.flags		= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY;
	s_mods.list.generic_mc.callback	= UI_Mods_MenuEvent;
	s_mods.list.generic_mc.id			= ID_LIST;
	s_mods.list.generic_mc.x			= 320;
	s_mods.list.generic_mc.y			= 130;
