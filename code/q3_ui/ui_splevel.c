...
static void UI_SPLevelMenu_SetMenuArena( int n, int level, const char *arenaInfo ) {
	...
	if ( selectedArenaSet > currentSet ) {
		levelMenuInfo.item_maps[n].generic_mc.flags |= QMF_GRAYED;
	}
	else {
		levelMenuInfo.item_maps[n].generic_mc.flags &= ~QMF_GRAYED;
	}

	levelMenuInfo.item_maps[n].generic_mc.flags &= ~QMF_INACTIVE;
}
...
static void UI_SPLevelMenu_SetMenuItems( void ) {
	...
	if( selectedArenaSet == trainingTier ) {
		...
		levelMenuInfo.item_maps[0].generic_mc.x = 256;
		Bitmap_Init( &levelMenuInfo.item_maps[0] );
		levelMenuInfo.item_maps[0].generic_mc.bottom += 32;
		levelMenuInfo.numMaps = 1;

		levelMenuInfo.item_maps[1].generic_mc.flags |= QMF_INACTIVE;
		levelMenuInfo.item_maps[2].generic_mc.flags |= QMF_INACTIVE;
		levelMenuInfo.item_maps[3].generic_mc.flags |= QMF_INACTIVE;
		...
		levelMenuInfo.item_maps[0].generic_mc.x = 46;
		Bitmap_Init( &levelMenuInfo.item_maps[0] );
		levelMenuInfo.item_maps[0].generic_mc.bottom += 18;
	...
	if ( selectedArenaSet == minTier ) {
		levelMenuInfo.item_leftarrow.generic_mc.flags |= ( QMF_INACTIVE | QMF_HIDDEN );
	}
	else {
		levelMenuInfo.item_leftarrow.generic_mc.flags &= ~( QMF_INACTIVE | QMF_HIDDEN );
	}
	...
	if ( selectedArenaSet == maxTier ) {
		levelMenuInfo.item_rightarrow.generic_mc.flags |= ( QMF_INACTIVE | QMF_HIDDEN );
	}
	else {
		levelMenuInfo.item_rightarrow.generic_mc.flags &= ~( QMF_INACTIVE | QMF_HIDDEN );
	}
...
//	int				fraglimit;
	...
	for ( n = 0; n < levelMenuInfo.numMaps; n++ ) {
		x = levelMenuInfo.item_maps[n].generic_mc.x;
		y = levelMenuInfo.item_maps[n].generic_mc.y;
	...
	color[3] = 0.5+0.5*sin((float)(uis.realtime/PULSE_DIVISOR)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	for ( n = 0; n < levelMenuInfo.numMaps; n++ ) {
		x = levelMenuInfo.item_maps[n].generic_mc.x;
		y = levelMenuInfo.item_maps[n].generic_mc.y;
...
static void UI_SPLevelMenu_Init( void ) {
	...
	levelMenuInfo.item_banner.generic_mc.type			= MTYPE_BTEXT;
	levelMenuInfo.item_banner.generic_mc.x				= 320;
	levelMenuInfo.item_banner.generic_mc.y				= 16;
	...
	levelMenuInfo.item_leftarrow.generic_mc.type		= MTYPE_BITMAP;
	levelMenuInfo.item_leftarrow.generic_mc.name		= ART_ARROW;
	levelMenuInfo.item_leftarrow.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_leftarrow.generic_mc.x			= 18;
	levelMenuInfo.item_leftarrow.generic_mc.y			= 64;
	levelMenuInfo.item_leftarrow.generic_mc.callback	= UI_SPLevelMenu_LeftArrowEvent;
	levelMenuInfo.item_leftarrow.generic_mc.id			= ID_LEFTARROW;
	...
	levelMenuInfo.item_maps[0].generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_maps[0].generic_mc.name			= levelMenuInfo.levelPicNames[0];
	levelMenuInfo.item_maps[0].generic_mc.flags		= QMF_LEFT_JUSTIFY;
	levelMenuInfo.item_maps[0].generic_mc.x			= 46;
	levelMenuInfo.item_maps[0].generic_mc.y			= 64;
	levelMenuInfo.item_maps[0].generic_mc.id			= ID_PICTURE0;
	levelMenuInfo.item_maps[0].generic_mc.callback		= UI_SPLevelMenu_LevelEvent;
	levelMenuInfo.item_maps[0].width				= 128;
	levelMenuInfo.item_maps[0].height				= 96;

	levelMenuInfo.item_maps[1].generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_maps[1].generic_mc.name			= levelMenuInfo.levelPicNames[1];
	levelMenuInfo.item_maps[1].generic_mc.flags		= QMF_LEFT_JUSTIFY;
	levelMenuInfo.item_maps[1].generic_mc.x			= 186;
	levelMenuInfo.item_maps[1].generic_mc.y			= 64;
	levelMenuInfo.item_maps[1].generic_mc.id			= ID_PICTURE1;
	levelMenuInfo.item_maps[1].generic_mc.callback		= UI_SPLevelMenu_LevelEvent;
	levelMenuInfo.item_maps[1].width				= 128;
	levelMenuInfo.item_maps[1].height				= 96;

	levelMenuInfo.item_maps[2].generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_maps[2].generic_mc.name			= levelMenuInfo.levelPicNames[2];
	levelMenuInfo.item_maps[2].generic_mc.flags		= QMF_LEFT_JUSTIFY;
	levelMenuInfo.item_maps[2].generic_mc.x			= 326;
	levelMenuInfo.item_maps[2].generic_mc.y			= 64;
	levelMenuInfo.item_maps[2].generic_mc.id			= ID_PICTURE2;
	levelMenuInfo.item_maps[2].generic_mc.callback		= UI_SPLevelMenu_LevelEvent;
	levelMenuInfo.item_maps[2].width				= 128;
	levelMenuInfo.item_maps[2].height				= 96;

	levelMenuInfo.item_maps[3].generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_maps[3].generic_mc.name			= levelMenuInfo.levelPicNames[3];
	levelMenuInfo.item_maps[3].generic_mc.flags		= QMF_LEFT_JUSTIFY;
	levelMenuInfo.item_maps[3].generic_mc.x			= 466;
	levelMenuInfo.item_maps[3].generic_mc.y			= 64;
	levelMenuInfo.item_maps[3].generic_mc.id			= ID_PICTURE3;
	levelMenuInfo.item_maps[3].generic_mc.callback		= UI_SPLevelMenu_LevelEvent;
	levelMenuInfo.item_maps[3].width				= 128;
	levelMenuInfo.item_maps[3].height				= 96;

	levelMenuInfo.item_rightarrow.generic_mc.type		= MTYPE_BITMAP;
	levelMenuInfo.item_rightarrow.generic_mc.name		= ART_ARROW;
	levelMenuInfo.item_rightarrow.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_rightarrow.generic_mc.x			= 606;
	levelMenuInfo.item_rightarrow.generic_mc.y			= 64;
	levelMenuInfo.item_rightarrow.generic_mc.callback	= UI_SPLevelMenu_RightArrowEvent;
	levelMenuInfo.item_rightarrow.generic_mc.id		= ID_RIGHTARROW;
	...
	PlayerIcon( levelMenuInfo.playerModel, levelMenuInfo.playerPicName, sizeof(levelMenuInfo.playerPicName) );
	levelMenuInfo.item_player.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_player.generic_mc.name			= levelMenuInfo.playerPicName;
	levelMenuInfo.item_player.generic_mc.flags			= QMF_LEFT_JUSTIFY|QMF_MOUSEONLY;
	levelMenuInfo.item_player.generic_mc.x				= 288;
	levelMenuInfo.item_player.generic_mc.y				= AWARDS_Y;
	levelMenuInfo.item_player.generic_mc.id			= ID_PLAYERPIC;
	levelMenuInfo.item_player.generic_mc.callback		= UI_SPLevelMenu_PlayerEvent;
	...
	for( n = 0; n < 6; n++ ) {
		if( levelMenuInfo.awardLevels[n] ) {
			...
			levelMenuInfo.item_awards[count].generic_mc.type		= MTYPE_BITMAP;
			levelMenuInfo.item_awards[count].generic_mc.name		= ui_medalPicNames[n];
			levelMenuInfo.item_awards[count].generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_SILENT|QMF_MOUSEONLY;
			levelMenuInfo.item_awards[count].generic_mc.x			= x;
			levelMenuInfo.item_awards[count].generic_mc.y			= y;
			levelMenuInfo.item_awards[count].generic_mc.id			= ID_AWARD1 + n;
			levelMenuInfo.item_awards[count].generic_mc.callback	= UI_SPLevelMenu_AwardEvent;
	...
	levelMenuInfo.item_back.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_back.generic_mc.name			= ART_BACK0;
	levelMenuInfo.item_back.generic_mc.flags			= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_back.generic_mc.x				= 0;
	levelMenuInfo.item_back.generic_mc.y				= 480-64;
	levelMenuInfo.item_back.generic_mc.callback		= UI_SPLevelMenu_BackEvent;
	levelMenuInfo.item_back.generic_mc.id				= ID_BACK;
	levelMenuInfo.item_back.width					= 128;
	levelMenuInfo.item_back.height					= 64;
	levelMenuInfo.item_back.focuspic				= ART_BACK1;

	levelMenuInfo.item_reset.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_reset.generic_mc.name			= ART_RESET0;
	levelMenuInfo.item_reset.generic_mc.flags			= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_reset.generic_mc.x				= 170;
	levelMenuInfo.item_reset.generic_mc.y				= 480-64;
	levelMenuInfo.item_reset.generic_mc.callback		= UI_SPLevelMenu_ResetEvent;
	levelMenuInfo.item_reset.generic_mc.id				= ID_RESET;
	levelMenuInfo.item_reset.width					= 128;
	levelMenuInfo.item_reset.height					= 64;
	levelMenuInfo.item_reset.focuspic				= ART_RESET1;

	levelMenuInfo.item_custom.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_custom.generic_mc.name			= ART_CUSTOM0;
	levelMenuInfo.item_custom.generic_mc.flags			= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_custom.generic_mc.x				= 342;
	levelMenuInfo.item_custom.generic_mc.y				= 480-64;
	levelMenuInfo.item_custom.generic_mc.callback		= UI_SPLevelMenu_CustomEvent;
	levelMenuInfo.item_custom.generic_mc.id			= ID_CUSTOM;
	levelMenuInfo.item_custom.width					= 128;
	levelMenuInfo.item_custom.height				= 64;
	levelMenuInfo.item_custom.focuspic				= ART_CUSTOM1;

	levelMenuInfo.item_next.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_next.generic_mc.name			= ART_FIGHT0;
	levelMenuInfo.item_next.generic_mc.flags			= QMF_RIGHT_JUSTIFY|QMF_PULSEIFFOCUS;
	levelMenuInfo.item_next.generic_mc.x				= 640;
	levelMenuInfo.item_next.generic_mc.y				= 480-64;
	levelMenuInfo.item_next.generic_mc.callback		= UI_SPLevelMenu_NextEvent;
	levelMenuInfo.item_next.generic_mc.id				= ID_NEXT;
	levelMenuInfo.item_next.width					= 128;
	levelMenuInfo.item_next.height					= 64;
	levelMenuInfo.item_next.focuspic				= ART_FIGHT1;

	levelMenuInfo.item_null.generic_mc.type			= MTYPE_BITMAP;
	levelMenuInfo.item_null.generic_mc.flags			= QMF_LEFT_JUSTIFY|QMF_MOUSEONLY|QMF_SILENT;
	levelMenuInfo.item_null.generic_mc.x				= 0;
	levelMenuInfo.item_null.generic_mc.y				= 0;
	...
	levelMenuInfo.item_maps[0].generic_mc.bottom += 18;
	levelMenuInfo.item_maps[1].generic_mc.bottom += 18;
	levelMenuInfo.item_maps[2].generic_mc.bottom += 18;
	levelMenuInfo.item_maps[3].generic_mc.bottom += 18;
