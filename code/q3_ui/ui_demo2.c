...
static sfxHandle_t UI_DemosMenu_Key( int key ) {
	...
	item = (menucommon_s *)Menu_ItemAtCursor( &s_demos.menu ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
static void Demos_MenuInit( void ) {
	...
	s_demos.banner.generic_mc.type		= MTYPE_BTEXT;
	s_demos.banner.generic_mc.x		= 320;
	s_demos.banner.generic_mc.y		= 16;
	s_demos.banner.string			= "DEMOS";
	s_demos.banner.color			= color_white;
	s_demos.banner.style			= UI_CENTER;

	s_demos.framel.generic_mc.type		= MTYPE_BITMAP;
	s_demos.framel.generic_mc.name		= ART_FRAMEL;
	s_demos.framel.generic_mc.flags	= QMF_INACTIVE;
	s_demos.framel.generic_mc.x		= 0;  
	s_demos.framel.generic_mc.y		= 78;
	s_demos.framel.width			= 256;
	s_demos.framel.height			= 329;

	s_demos.framer.generic_mc.type		= MTYPE_BITMAP;
	s_demos.framer.generic_mc.name		= ART_FRAMER;
	s_demos.framer.generic_mc.flags	= QMF_INACTIVE;
	s_demos.framer.generic_mc.x		= 376;
	s_demos.framer.generic_mc.y		= 76;
	s_demos.framer.width			= 256;
	s_demos.framer.height			= 334;

	s_demos.arrows.generic_mc.type		= MTYPE_BITMAP;
	s_demos.arrows.generic_mc.name		= ART_ARROWS;
	s_demos.arrows.generic_mc.flags	= QMF_INACTIVE;
	s_demos.arrows.generic_mc.x		= 320-ARROWS_WIDTH/2;
	s_demos.arrows.generic_mc.y		= 400;
	s_demos.arrows.width			= ARROWS_WIDTH;
	s_demos.arrows.height			= ARROWS_HEIGHT;

	s_demos.left.generic_mc.type		= MTYPE_BITMAP;
	s_demos.left.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_MOUSEONLY;
	s_demos.left.generic_mc.x			= 320-ARROWS_WIDTH/2;
	s_demos.left.generic_mc.y			= 400;
	s_demos.left.generic_mc.id			= ID_LEFT;
	s_demos.left.generic_mc.callback	= Demos_MenuEvent;
	s_demos.left.width				= ARROWS_WIDTH/2;
	s_demos.left.height				= ARROWS_HEIGHT;
	s_demos.left.focuspic			= ART_ARROWLEFT;

	s_demos.right.generic_mc.type		= MTYPE_BITMAP;
	s_demos.right.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_MOUSEONLY;
	s_demos.right.generic_mc.x			= 320;
	s_demos.right.generic_mc.y			= 400;
	s_demos.right.generic_mc.id		= ID_RIGHT;
	s_demos.right.generic_mc.callback	= Demos_MenuEvent;
	s_demos.right.width				= ARROWS_WIDTH/2;
	s_demos.right.height			= ARROWS_HEIGHT;
	s_demos.right.focuspic			= ART_ARROWRIGHT;

	s_demos.back.generic_mc.type		= MTYPE_BITMAP;
	s_demos.back.generic_mc.name		= ART_BACK0;
	s_demos.back.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_demos.back.generic_mc.id			= ID_BACK;
	s_demos.back.generic_mc.callback	= Demos_MenuEvent;
	s_demos.back.generic_mc.x			= 0;
	s_demos.back.generic_mc.y			= 480-64;
	s_demos.back.width				= 128;
	s_demos.back.height				= 64;
	s_demos.back.focuspic			= ART_BACK1;

	s_demos.go.generic_mc.type			= MTYPE_BITMAP;
	s_demos.go.generic_mc.name			= ART_GO0;
	s_demos.go.generic_mc.flags		= QMF_RIGHT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_demos.go.generic_mc.id			= ID_GO;
	s_demos.go.generic_mc.callback		= Demos_MenuEvent;
	s_demos.go.generic_mc.x			= 640;
	s_demos.go.generic_mc.y			= 480-64;
	s_demos.go.width				= 128;
	s_demos.go.height				= 64;
	s_demos.go.focuspic				= ART_GO1;

	s_demos.list.generic_mc.type		= MTYPE_SCROLLLIST;
	s_demos.list.generic_mc.flags		= QMF_PULSEIFFOCUS;
	s_demos.list.generic_mc.callback	= Demos_MenuEvent;
	s_demos.list.generic_mc.id			= ID_LIST;
	s_demos.list.generic_mc.x			= 118;
	s_demos.list.generic_mc.y			= 130;
	...
	if (!s_demos.list.numitems) {
		...
		s_demos.go.generic_mc.flags |= (QMF_INACTIVE|QMF_HIDDEN);
	}
