...
void UI_ResetMenu(void) {
	...
	s_reset.yes.generic_mc.type		= MTYPE_PTEXT;      
	s_reset.yes.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS; 
	s_reset.yes.generic_mc.callback	= Reset_MenuEvent;
	s_reset.yes.generic_mc.id			= ID_YES;
	s_reset.yes.generic_mc.x			= l1;
	s_reset.yes.generic_mc.y			= 264;
	s_reset.yes.string				= "YES";
	s_reset.yes.color				= color_red;
	s_reset.yes.style				= UI_LEFT;

	s_reset.no.generic_mc.type			= MTYPE_PTEXT;      
	s_reset.no.generic_mc.flags		= QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS; 
	s_reset.no.generic_mc.callback		= Reset_MenuEvent;
	s_reset.no.generic_mc.id			= ID_NO;
	s_reset.no.generic_mc.x		    = l3;
	s_reset.no.generic_mc.y		    = 264;
