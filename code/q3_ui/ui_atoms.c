...
void UI_DrawProportionalString( int x, int y, const char* str, int style, vec4_t color ) {
	...
	if ( style & UI_PULSE ) {
		...
		drawcolor[1] = color[1];
		drawcolor[2] = color[2];
		drawcolor[3] = 0.5 + 0.5 * sin( (float)(uis.realtime / PULSE_DIVISOR) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void UI_DrawString( int x, int y, const char* str, int style, vec4_t color )
{
	...
	if (style & UI_PULSE)
	{
		...
		UI_LerpColor(color,lowlight,newcolor,0.5+0.5*sin((float)(uis.realtime/PULSE_DIVISOR))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		drawcolor = newcolor;
	}	
	else
		drawcolor = color;
