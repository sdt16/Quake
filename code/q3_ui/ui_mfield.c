...
void MField_KeyDownEvent( mfield_t *edit, int key ) {
	...
	if ( key == K_INS || key == K_KP_INS ) {
		trap_Key_SetOverstrikeMode( (qboolean)(!trap_Key_GetOverstrikeMode()) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		return;
	}
}
...
void MenuField_Init( menufield_s* m ) {
	...
	if (m->generic_mc.flags & QMF_SMALLFONT)
	{
		w = SMALLCHAR_WIDTH;
		h = SMALLCHAR_HEIGHT;
	}
	...
	if (m->generic_mc.name) {
		l = (strlen( m->generic_mc.name )+1) * w;		
	}
	else {
		l = 0;
	}

	m->generic_mc.left   = m->generic_mc.x - l;
	m->generic_mc.top    = m->generic_mc.y;
	m->generic_mc.right  = m->generic_mc.x + w + m->field.widthInChars*w;
	m->generic_mc.bottom = m->generic_mc.y + h;
}
...
void MenuField_Draw( menufield_s *f )
{
	...
	if (f->generic_mc.flags & QMF_SMALLFONT)
	{
		w = SMALLCHAR_WIDTH;
		h = SMALLCHAR_HEIGHT;
		style = UI_SMALLFONT;
	...
	if (Menu_ItemAtCursor( f->generic_mc.parent ) == f) {
		focus = qtrue;
		style |= UI_PULSE;
	}
	else {
	...
	if (f->generic_mc.flags & QMF_GRAYED)
	...
	{
		// draw cursor
		UI_FillRect( f->generic_mc.left, f->generic_mc.top, f->generic_mc.right-f->generic_mc.left+1, f->generic_mc.bottom-f->generic_mc.top+1, listbar_color ); 
		UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|style, color);
	}

	if ( f->generic_mc.name ) {
		UI_DrawString( x - w, y, f->generic_mc.name, style|UI_RIGHT, color );
	}
	...
	switch ( keycode )
	{
		...
		default:
			if ( keycode & K_CHAR_FLAG )
			{
				...
				if ((m->generic_mc.flags & QMF_UPPERCASE) && Q_islower( keycode ))
					keycode -= 'a' - 'A';
				else if ((m->generic_mc.flags & QMF_LOWERCASE) && Q_isupper( keycode ))
					keycode -= 'A' - 'a';
				else if ((m->generic_mc.flags & QMF_NUMBERSONLY) && Q_isalpha( keycode ))
