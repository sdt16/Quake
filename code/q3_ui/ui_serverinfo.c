...
void UI_ServerInfoMenu( void )
{
	...
	s_serverinfo.banner.generic_mc.type  = MTYPE_BTEXT;
	s_serverinfo.banner.generic_mc.x	  = 320;
	s_serverinfo.banner.generic_mc.y	  = 16;
	s_serverinfo.banner.string		  = "SERVER INFO";
	s_serverinfo.banner.color	      = color_white;
	s_serverinfo.banner.style	      = UI_CENTER;

	s_serverinfo.framel.generic_mc.type  = MTYPE_BITMAP;
	s_serverinfo.framel.generic_mc.name  = SERVERINFO_FRAMEL;
	s_serverinfo.framel.generic_mc.flags = QMF_INACTIVE;
	s_serverinfo.framel.generic_mc.x	  = 0;  
	s_serverinfo.framel.generic_mc.y	  = 78;
	s_serverinfo.framel.width  	      = 256;
	s_serverinfo.framel.height  	  = 329;

	s_serverinfo.framer.generic_mc.type  = MTYPE_BITMAP;
	s_serverinfo.framer.generic_mc.name  = SERVERINFO_FRAMER;
	s_serverinfo.framer.generic_mc.flags = QMF_INACTIVE;
	s_serverinfo.framer.generic_mc.x	  = 376;
	s_serverinfo.framer.generic_mc.y	  = 76;
	s_serverinfo.framer.width  	      = 256;
	s_serverinfo.framer.height  	  = 334;

	s_serverinfo.add.generic_mc.type	  = MTYPE_PTEXT;
	s_serverinfo.add.generic_mc.flags    = QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_serverinfo.add.generic_mc.callback = ServerInfo_Event;
	s_serverinfo.add.generic_mc.id	      = ID_ADD;
	s_serverinfo.add.generic_mc.x		  = 320;
	s_serverinfo.add.generic_mc.y		  = 371;
	...
	if( trap_Cvar_VariableValue( "sv_running" ) ) {
		s_serverinfo.add.generic_mc.flags |= QMF_GRAYED;
	}

	s_serverinfo.back.generic_mc.type	   = MTYPE_BITMAP;
	s_serverinfo.back.generic_mc.name     = SERVERINFO_BACK0;
	s_serverinfo.back.generic_mc.flags    = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_serverinfo.back.generic_mc.callback = ServerInfo_Event;
	s_serverinfo.back.generic_mc.id	   = ID_BACK;
	s_serverinfo.back.generic_mc.x		   = 0;
	s_serverinfo.back.generic_mc.y		   = 480-64;
