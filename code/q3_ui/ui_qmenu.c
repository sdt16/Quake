...
{
	t->generic_mc.flags |= QMF_INACTIVE;
}
...
static void Text_Draw( menutext_s *t )
{
	...
	// possible label
	if (t->generic_mc.name)
		strcpy(buff,t->generic_mc.name);

	// possible value
	if (t->string)
		strcat(buff,t->string);
		
	if (t->generic_mc.flags & QMF_GRAYED)
...
static void PText_Init( menutext_s *t )
{
	...
	if( t->generic_mc.flags & QMF_RIGHT_JUSTIFY ) {
		x -= w;
	}
	else if( t->generic_mc.flags & QMF_CENTER_JUSTIFY ) {
		x -= w / 2;
	}

	t->generic_mc.left   = x - PROP_GAP_WIDTH * sizeScale;
	t->generic_mc.right  = x + w + PROP_GAP_WIDTH * sizeScale;
	t->generic_mc.top    = y;
	t->generic_mc.bottom = y + h;
}
...
static void PText_Draw( menutext_s *t )
{
	...
	style = t->style;
	if( t->generic_mc.flags & QMF_PULSEIFFOCUS ) {
		if( Menu_ItemAtCursor( t->generic_mc.parent ) == t ) {
			style |= UI_PULSE;
		}
		else {
			style |= UI_INVERSE;
...
void Bitmap_Init( menubitmap_s *b )
{
	...
	if (b->generic_mc.flags & QMF_RIGHT_JUSTIFY)
	{
		x = x - w;
	}
	else if (b->generic_mc.flags & QMF_CENTER_JUSTIFY)
	{
		x = x - w/2;
	}

	b->generic_mc.left   = x;
	b->generic_mc.right  = x + w;
	b->generic_mc.top    = y;
	b->generic_mc.bottom = y + h;
...
void Bitmap_Draw( menubitmap_s *b )
{
	...
	if (b->generic_mc.flags & QMF_GRAYED)
	{
		if (b->shader)
		{
	...
	else
	{
		...
		if (  ( (b->generic_mc.flags & QMF_PULSE) 
			|| (b->generic_mc.flags & QMF_PULSEIFFOCUS) )
		      && (Menu_ItemAtCursor( b->generic_mc.parent ) == b))
		{	
			if (b->focuscolor)			
			{
		...
		else if ((b->generic_mc.flags & QMF_HIGHLIGHT) || ((b->generic_mc.flags & QMF_HIGHLIGHT_IF_FOCUS) && (Menu_ItemAtCursor( b->generic_mc.parent ) == b)))
		{	
			if (b->focuscolor)
			{
...
static void Action_Init( menuaction_s *a )
{
	...
	// calculate bounds
	if (a->generic_mc.name)
		len = strlen(a->generic_mc.name);
	else
		len = 0;

	// left justify text
	a->generic_mc.left   = a->generic_mc.x; 
	a->generic_mc.right  = a->generic_mc.x + len*BIGCHAR_WIDTH;
	a->generic_mc.top    = a->generic_mc.y;
	a->generic_mc.bottom = a->generic_mc.y + BIGCHAR_HEIGHT;
}
...
static void Action_Draw( menuaction_s *a )
{
	...
	else if (( a->generic_mc.flags & QMF_PULSEIFFOCUS ) && ( a->generic_mc.parent->cursor == a->generic_mc.menuPosition ))
	{
		color = text_color_highlight;
		style = UI_PULSE;
	}
	else if (( a->generic_mc.flags & QMF_HIGHLIGHT_IF_FOCUS ) && ( a->generic_mc.parent->cursor == a->generic_mc.menuPosition ))
	{
		color = text_color_highlight;
	}
	else if ( a->generic_mc.flags & QMF_BLINK )
	{
		style = UI_BLINK;
		color = text_color_highlight;
	}
	...
	UI_DrawString( x, y, a->generic_mc.name, UI_LEFT|style, color );

	if ( a->generic_mc.parent->cursor == a->generic_mc.menuPosition )
	{
		// draw cursor
...
static void RadioButton_Init( menuradiobutton_s *rb )
{
	...
	// calculate bounds
	if (rb->generic_mc.name)
		len = strlen(rb->generic_mc.name);
	else
		len = 0;

	rb->generic_mc.left   = rb->generic_mc.x - (len+1)*SMALLCHAR_WIDTH;
	rb->generic_mc.right  = rb->generic_mc.x + 6*SMALLCHAR_WIDTH;
	rb->generic_mc.top    = rb->generic_mc.y;
	rb->generic_mc.bottom = rb->generic_mc.y + SMALLCHAR_HEIGHT;
}
...
static sfxHandle_t RadioButton_Key( menuradiobutton_s *rb, int key )
{
	...
	{
		case K_MOUSE1:
			if (!(rb->generic_mc.flags & QMF_HASMOUSEFOCUS))
				break;
		...
		case K_RIGHTARROW:
			...
			if ( rb->generic_mc.callback )
				rb->generic_mc.callback( rb, QM_ACTIVATED );
...
static void RadioButton_Draw( menuradiobutton_s *rb )
{
	...
	focus = (qboolean)(rb->generic_mc.parent->cursor == rb->generic_mc.menuPosition); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)

	if ( rb->generic_mc.flags & QMF_GRAYED )
	{
		color = text_color_disabled;
	...
	{
		// draw cursor
		UI_FillRect( rb->generic_mc.left, rb->generic_mc.top, rb->generic_mc.right-rb->generic_mc.left+1, rb->generic_mc.bottom-rb->generic_mc.top+1, listbar_color ); 
		UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|UI_SMALLFONT, color);
	}

	if ( rb->generic_mc.name )
		UI_DrawString( x - SMALLCHAR_WIDTH, y, rb->generic_mc.name, UI_RIGHT|UI_SMALLFONT, color );
...
static void Slider_Init( menuslider_s *s )
{
	...
	// calculate bounds
	if (s->generic_mc.name)
		len = strlen(s->generic_mc.name);
	else
		len = 0;

	s->generic_mc.left   = s->generic_mc.x - (len+1)*SMALLCHAR_WIDTH; 
	s->generic_mc.right  = s->generic_mc.x + (SLIDER_RANGE+2+1)*SMALLCHAR_WIDTH;
	s->generic_mc.top    = s->generic_mc.y;
	s->generic_mc.bottom = s->generic_mc.y + SMALLCHAR_HEIGHT;
}
...
static sfxHandle_t Slider_Key( menuslider_s *s, int key )
{
	...
	{
		case K_MOUSE1:
			x           = uis.cursorx - s->generic_mc.x - 2*SMALLCHAR_WIDTH;
			oldvalue    = s->curvalue;
	...
	if ( sound && s->generic_mc.callback )
		s->generic_mc.callback( s, QM_ACTIVATED );
...
static void Slider_Draw( menuslider_s *s ) {
	...
	// draw label
	UI_DrawString( x - SMALLCHAR_WIDTH, y, s->generic_mc.name, UI_RIGHT|style, color );
...
static void Slider_Draw( menuslider_s *s )
{
	...
	x =	s->generic_mc.x;
	y = s->generic_mc.y;
	focus = (s->generic_mc.parent->cursor == s->generic_mc.menuPosition);
...
static void SpinControl_Init( menulist_s *s ) {
	...
	if (s->generic_mc.name)
		len = strlen(s->generic_mc.name) * SMALLCHAR_WIDTH;
	else
		len = 0;

	s->generic_mc.left	= s->generic_mc.x - SMALLCHAR_WIDTH - len;
	...
	s->generic_mc.top	  =	s->generic_mc.y;
	s->generic_mc.right  =	s->generic_mc.x + (len+1)*SMALLCHAR_WIDTH;
	s->generic_mc.bottom =	s->generic_mc.y + SMALLCHAR_HEIGHT;
}
...
static void ScrollList_Init( menulist_s *l )
{
	...
	l->generic_mc.left   =	l->generic_mc.x;
	l->generic_mc.top    = l->generic_mc.y;	
	l->generic_mc.right  =	l->generic_mc.x + w;
	l->generic_mc.bottom =	l->generic_mc.y + l->height * SMALLCHAR_HEIGHT;

	if( l->generic_mc.flags & QMF_CENTER_JUSTIFY ) {
		l->generic_mc.left -= w / 2;
		l->generic_mc.right -= w / 2;
	}
}
...
sfxHandle_t ScrollList_Key( menulist_s *l, int key )
{
	...
	{
		case K_MOUSE1:
			if (l->generic_mc.flags & QMF_HASMOUSEFOCUS)
			{
				// check scroll region
				x = l->generic_mc.x;
				y = l->generic_mc.y;
				...
				if (UI_CursorInRect( x, y, w, l->height*SMALLCHAR_HEIGHT ))
				{
					...
					if (l->top + index < l->numitems)
					{
						...
						if (l->oldvalue != l->curvalue && l->generic_mc.callback)
						{
							l->generic_mc.callback( l, QM_GOTFOCUS );
							return (menu_move_sound);
						}
					}
				}
...
void Menu_Draw( menuframework_s *menu )
{
	...
	itemptr = (menucommon_s *)Menu_ItemAtCursor( menu ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
sfxHandle_t Menu_DefaultKey( menuframework_s *m, int key )
{
	...
	item = (menucommon_s *)Menu_ItemAtCursor( m ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	switch ( key )
	{
		...
		case K_F11:
			//uis.debug ^= 1;
			uis.debug = (qboolean)(((int)(uis.debug))^1); // ***GREGS_VC9_PORT_MOD*** -- legal way to simulate "uis.debug ^= 1;"
			break;
