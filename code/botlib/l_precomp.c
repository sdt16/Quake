...
//		{ "__STDC__", BUILTIN_STDC },
	...
	*/
	...
	struct builtin_s builtin[] = { // bk001204 - brackets
		{ "__LINE__",	BUILTIN_LINE },
		{ "__FILE__",	BUILTIN_FILE },
		{ "__DATE__",	BUILTIN_DATE },
		{ "__TIME__",	BUILTIN_TIME },
										...
										token_t **firsttoken, token_t **lasttoken)
{
	...
	switch(define->builtin)
	{
		...
		{
			t = time(NULL);
			curtime = ctime((const time_t *)&t); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
			strcpy(token->string, "\"");
...
#if DEFINEHASHING
	src.definehash = (define_t **)GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t *)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
#endif //DEFINEHASHING
...
{
	//int operator;
	int op; // ***GREGS_VC9_PORT_MOD*** -- "operator" is a keyword, so had to change this member to "op"
	int priority;
	int parentheses;
																	...
																	double *floatvalue, int integer)
{
		...
		switch(t->type)
		{
			...
			case TT_PUNCTUATION:
			{
				...
				if (!error && !negativevalue)
				{
					...
					AllocOperator(o);
					o->op = t->subtype; // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
	...
	while(!error && firstoperator)
	{
		...
		for (o = firstoperator; o->next; o = o->next)
		{
			...
			if (o->op != P_LOGIC_NOT // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
					&& o->op != P_BIN_NOT) v = v->next; // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
...
#endif //DEBUG_EVAL
		switch(o->op) // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
		{
...
#endif //DEBUG_EVAL
		if (error) break;
		lastoperatortype = o->op; // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
		//if not an operator with arity 1
		if (o->op != P_LOGIC_NOT 
				&& o->op != P_BIN_NOT) // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
		{
			//remove the second value if not question mark operator
			if (o->op != P_QUESTIONMARK) v = v->next; // ***GREGS_VC9_PORT_MOD*** -- changed "operator" to "op
			//
...
#if DEFINEHASHING
	source->definehash = (define_t **)GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t *)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
#endif //DEFINEHASHING
