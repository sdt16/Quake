...
qboolean trap_Key_IsDown( int keynum ) {
	return (qboolean)syscall( UI_KEY_ISDOWN, keynum ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}

qboolean trap_Key_GetOverstrikeMode( void ) {
	return (qboolean)syscall( UI_KEY_GETOVERSTRIKEMODE ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
qboolean trap_LAN_UpdateVisiblePings( int source ) {
	return (qboolean)syscall( UI_LAN_UPDATEVISIBLEPINGS, source ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
e_status trap_CIN_StopCinematic(int handle) {
  return (e_status)syscall(UI_CIN_STOPCINEMATIC, handle); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}


// will run a frame of the cinematic but will not draw it.  Will return FMV_EOF if the end of the cinematic has been reached.
e_status trap_CIN_RunCinematic (int handle) {
  return (e_status)syscall(UI_CIN_RUNCINEMATIC, handle); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
qboolean trap_VerifyCDKey( const char *key, const char *chksum) {
	return (qboolean)syscall( UI_VERIFY_CDKEY, key, chksum); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
