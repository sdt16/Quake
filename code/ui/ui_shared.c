...
qboolean UI_OutOfMemory() {
	return (qboolean)outOfMemory; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
const char *String_Alloc(const char *p) {
	...
	if (len + strPoolIndex + 1 < STRING_POOL_SIZE) {
		...
		str  = (stringDef_t *)UI_Alloc(sizeof(stringDef_t)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		str->next = NULL;
		str->str = &strPool[ph];
		if (last) {
			last->next = str;
...
void Item_UpdatePosition(itemDef_t *item) {
  ...
  menu = (menuDef_t *)item->parent; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
qboolean IsVisible(int flags) {
  return (qboolean)(flags & WINDOW_VISIBLE && !(flags & WINDOW_FADINGOUT)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
void Script_SetItemColor(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &itemname) && String_Parse(args, &name)) {
    itemDef_t *item2;
    int j;
    int count = Menu_ItemsMatchingGroup((menuDef_t *)item->parent, itemname); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
    ...
    for (j = 0; j < count; j++) {
      item2 = Menu_GetMatchingItemByNumber((menuDef_t *)item->parent, j, itemname); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
      if (item2 != NULL) {
        out = NULL;
        if (Q_stricmp(name, "backcolor") == 0) {
...
void Script_Show(itemDef_t *item, char **args) {
  const char *name;
  if (String_Parse(args, &name)) {
    Menu_ShowItemByName((menuDef_t *)item->parent, name, qtrue); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
  }
}
...
void Script_Hide(itemDef_t *item, char **args) {
  const char *name;
  if (String_Parse(args, &name)) {
    Menu_ShowItemByName((menuDef_t *)item->parent, name, qfalse); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
  }
}
...
void Script_FadeIn(itemDef_t *item, char **args) {
  const char *name;
  if (String_Parse(args, &name)) {
    Menu_FadeItemByName((menuDef_t *)item->parent, name, qfalse); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
  }
}
...
void Script_FadeOut(itemDef_t *item, char **args) {
  const char *name;
  if (String_Parse(args, &name)) {
    Menu_FadeItemByName((menuDef_t *)item->parent, name, qtrue); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
  }
}
...
void Script_Transition(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    if ( Rect_Parse(args, &rectFrom) && Rect_Parse(args, &rectTo) && Int_Parse(args, &time) && Float_Parse(args, &amt)) {
      Menu_TransitionItemByName((menuDef_t *)item->parent, name, rectFrom, rectTo, time, amt); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
    }
  }
}
...
void Script_Orbit(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    if ( Float_Parse(args, &x) && Float_Parse(args, &y) && Float_Parse(args, &cx) && Float_Parse(args, &cy) && Int_Parse(args, &time) ) {
      Menu_OrbitItemByName((menuDef_t *)item->parent, name, x, y, cx, cy, time); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
    }
  }
}
...
void Script_SetFocus(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    focusItem = Menu_FindItemByName((menuDef_t *)item->parent, name); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
    if (focusItem && !(focusItem->window.flags & WINDOW_DECORATION) && !(focusItem->window.flags & WINDOW_HASFOCUS)) {
      Menu_ClearFocus((menuDef_t *)item->parent); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
qboolean Item_SetFocus(itemDef_t *item, float x, float y) {
	...
	oldFocus = Menu_ClearFocus((menuDef_t *)item->parent); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
float Item_Slider_ThumbPosition(itemDef_t *item) {
	float value, range, x;
	editFieldDef_t *editDef = (editFieldDef_t *)item->typeData; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	if (item->cvar) {
		...
		if ( key & K_CHAR_FLAG ) {
			...
			if ( key == K_INS || key == K_KP_INS ) {
				DC->setOverstrikeMode((qboolean)(!DC->getOverstrikeMode())); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				return qtrue;
			}
		}

		if (key == K_TAB || key == K_DOWNARROW || key == K_KP_DOWNARROW) {
			newItem = Menu_SetNextCursorItem((menuDef_t *)item->parent); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		...
		if (key == K_UPARROW || key == K_KP_UPARROW) {
			newItem = Menu_SetPrevCursorItem((menuDef_t *)item->parent); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
			if (newItem && (newItem->type == ITEM_TYPE_EDITFIELD || newItem->type == ITEM_TYPE_NUMERICFIELD)) {
				g_editItem = newItem;
			}
		}
...
static void Scroll_Slider_ThumbFunc(void *p) {
	...
	scrollInfo_t *si = (scrollInfo_t*)p;
	editFieldDef_t *editDef = (editFieldDef_t *)si->item->typeData; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	switch ( key ) {
		...
		case K_F11:
			if (DC->getCVarValue("developer")) {
				//debugMode ^= 1;
				debugMode = (qboolean)(((int)debugMode)^1); // ***GREGS_VC9_PORT_MOD*** -- the operation above is not allowed in C++, must simulate it here
			}
			break;
...
void Item_TextColor(itemDef_t *item, vec4_t *newColor) {
	...
	if (item->window.flags & WINDOW_HASFOCUS) {
		...
		LerpColor(parent->focusColor,lowLight,*newColor,0.5+0.5*sin((float)(DC->realTime / PULSE_DIVISOR))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	} else if (item->textStyle == ITEM_TEXTSTYLE_BLINK && !((DC->realTime/BLINK_DIVISOR) & 1)) {
		lowLight[0] = 0.8 * item->window.foreColor[0]; 
		lowLight[1] = 0.8 * item->window.foreColor[1]; 
		lowLight[2] = 0.8 * item->window.foreColor[2]; 
		lowLight[3] = 0.8 * item->window.foreColor[3]; 
		LerpColor(item->window.foreColor,lowLight,*newColor,0.5+0.5*sin((float)(DC->realTime / PULSE_DIVISOR))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	} else {
  ...
  parent = (menuDef_t*)item->parent;
	...
	if (DC->ownerDrawItem) {
		...
		if (item->window.flags & WINDOW_HASFOCUS) {
			...
			lowLight[2] = 0.8 * parent->focusColor[2]; 
			lowLight[3] = 0.8 * parent->focusColor[3]; 
			LerpColor(parent->focusColor,lowLight,color,0.5+0.5*sin((float)(DC->realTime / PULSE_DIVISOR))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		} else if (item->textStyle == ITEM_TEXTSTYLE_BLINK && !((DC->realTime/BLINK_DIVISOR) & 1)) {
			lowLight[0] = 0.8 * item->window.foreColor[0]; 
			lowLight[1] = 0.8 * item->window.foreColor[1]; 
			lowLight[2] = 0.8 * item->window.foreColor[2]; 
			lowLight[3] = 0.8 * item->window.foreColor[3]; 
			LerpColor(item->window.foreColor,lowLight,color,0.5+0.5*sin((float)(DC->realTime / PULSE_DIVISOR))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		}
...
qboolean MenuParse_itemDef( itemDef_t *item, int handle ) {
	...
	if (menu->itemCount < MAX_MENUITEMS) {
		menu->items[menu->itemCount] = (itemDef_t *)UI_Alloc(sizeof(itemDef_t)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
qboolean Display_MouseMove(void *p, int x, int y) {
	int i;
	menuDef_t *menu = (menuDef_t *)p; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void Display_HandleKey(int key, qboolean down, int x, int y) {
	menuDef_t *menu = (menuDef_t *)Display_CaptureItem(x, y); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	if (menu == NULL) {  
		menu = Menu_GetFocused();
	}
	if (menu) {
