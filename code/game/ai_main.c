...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	if (!botstates[client]) botstates[client] = (bot_state_t *)G_Alloc(sizeof(bot_state_t)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	bs = botstates[client];
...
int BotAIShutdown( int restart ) {
	...
	if ( restart ) {
		...
		for (i = 0; i < MAX_CLIENTS; i++) {
			if (botstates[i] && botstates[i]->inuse) {
				BotAIShutdownClient(botstates[i]->client, (qboolean)restart); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
			}
		}
