...
qboolean BotIsDead(bot_state_t *bs) {
	return (qboolean)(bs->cur_ps.pm_type == PM_DEAD); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
qboolean BotIntermission(bot_state_t *bs) {
	...
	return (qboolean)(bs->cur_ps.pm_type == PM_FREEZE || bs->cur_ps.pm_type == PM_INTERMISSION); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
	...
	feet[2] -= 23;
	return (qboolean)(trap_AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
