...
#pragma warning(disable : 4996)		// ***GREGS_VC9_PORT_MOD*** -- get rid of "unsafe CRT function" warning
#pragma warning(disable : 4793)		// ***GREGS_VC9_PORT_MOD*** -- function compiled as native (inevitably, some functions have to be native - such as one's with assembly - so this warning is uncorrectable; thus we turn it off)
#endif
...
void MatrixMultiply(vec3_t *in1, vec3_t *in2, vec3_t *out); // ***GREGS_VC9_PORT_MOD*** -- changed from plain array types to vec3_t pointers (see function definition for reasons why)
