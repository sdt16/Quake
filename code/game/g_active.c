...
void P_WorldEffects( gentity_t *ent ) {
	...
	envirosuit = (qboolean)(ent->client->ps.powerups[PW_BATTLESUIT] > level.time); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void ClientIntermissionThink( gclient_t *client ) {
	...
	if ( client->buttons & ( BUTTON_ATTACK | BUTTON_USE_HOLDABLE ) & ( client->oldbuttons ^ client->buttons ) ) {
		...
		client->readyToExit = (qboolean)1; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	}
}
...
//		G_Printf("serverTime >>>>>\n" );
	...
	pm.noFootsteps = (qboolean)(( g_dmflags.integer & DF_NO_FOOTSTEPS ) > 0); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
