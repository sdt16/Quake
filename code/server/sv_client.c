...
gotnewcl:	
	...
	*newcl = temp;
	...
	if ( denied ) {
		...
		denied = (char *)VM_ExplicitArgPtr( gvm, (int)denied ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void SV_WriteDownloadToClient( client_t *cl , msg_t *msg )
{
	...
	while (cl->downloadCurrentBlock - cl->downloadClientBlock < MAX_DOWNLOAD_WINDOW &&
		...
		if (!cl->downloadBlocks[curindex])
			cl->downloadBlocks[curindex] = (unsigned char *)Z_Malloc( MAX_DOWNLOAD_BLKSIZE ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
static void SV_VerifyPaks_f( client_t *cl ) {
	...
	if ( sv_pure->integer != 0 ) {
		...
		bGood = (qboolean)(FS_FileIsInPAK("vm/cgame.qvm", &nChkSum1) == 1); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		if (bGood)
			bGood = (qboolean)(FS_FileIsInPAK("vm/ui.qvm", &nChkSum2) == 1); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
