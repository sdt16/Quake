...
void SV_ClipMoveToEntities( moveclip_t *clip ) {
	...
	for ( i=0 ; i<num ; i++ ) {
		...
		if ( trace.fraction < clip->trace.fraction ) {
			...
			trace.entityNum = touch->s.number;
			clip->trace = trace;
			//clip->trace.startsolid |= oldStart; // ***GREGS_VC9_PORT_MOD*** -- C++ doesn't allow this, but the line below simulates the same effect
			clip->trace.startsolid = (qboolean)((int)clip->trace.startsolid|(int)oldStart);
		}
	}
}
