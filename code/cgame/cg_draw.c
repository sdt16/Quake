...
//					((lwidth/2 - len/2) * TINYCHAR_WIDTH);
			...
			for (j = 0; j <= PW_NUM_POWERUPS; j++) {
				if (ci->powerups & (1 << j)) {
					...
					item = BG_FindItemForPowerup( (powerup_t)j ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
static float CG_DrawPowerups( float y ) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		item = BG_FindItemForPowerup( (powerup_t)sorted[i] ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
