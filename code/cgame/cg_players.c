...
void CG_NewClientInfo( int clientNum ) {
	...
	newInfo.team = (team_t)atoi( v ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	newInfo.teamLeader = (qboolean)atoi(v); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	if ( cg_forceModel.integer ) {
		...
		if ( cgs.gametype >= GT_TEAM ) {
			// keep skin name
			slash = (char *)strchr( v, '/' ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
			if ( slash ) {
	...
	if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		forceDefer = (qboolean)(trap_MemoryRemaining() < 4000000); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
