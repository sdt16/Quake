...
static void CG_ParseScores( void ) {
	...
	for ( i = 0 ; i < cg.numScores ; i++ ) {
		...
		cg.scores[i].perfect = (qboolean)(atoi(CG_Argv(i * 14 + 16))); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void CG_ParseServerinfo( void ) {
	...
	cgs.gametype = (gametype_t)atoi( Info_ValueForKey( info, "g_gametype" ) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void CG_ShaderStateChanged(void) {
	...
	while (o && *o) {
		n = (char *)strstr(o, "="); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		if (n && *n) {
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
		...
		cg.intermissionStarted = (qboolean)atoi( str ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
