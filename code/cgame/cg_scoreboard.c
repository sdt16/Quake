...
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxClients, int lineHeight ) {
	...
	for ( i = 0 ; i < cg.numScores && count < maxClients ; i++ ) {
		...
		CG_DrawClientScore( y + lineHeight * count, score, color, fade, (qboolean)(lineHeight == SB_NORMAL_HEIGHT) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	if (!localClient) {
		...
		for ( i = 0 ; i < cg.numScores ; i++ ) {
			if ( cg.scores[i].client == cg.snap->ps.clientNum ) {
				CG_DrawClientScore( y, &cg.scores[i], fadeColor, fade, (qboolean)(lineHeight == SB_NORMAL_HEIGHT) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				break;
			}
		}
	}
