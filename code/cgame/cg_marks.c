...
void CG_ImpactMark( qhandle_t markShader, const vec3_t origin, const vec3_t dir, 
	...
	numFragments = trap_CM_MarkFragments( 4, (vec3_t *)originalPoints, // ***GREGS_VC9_PORT_MOD*** -- changed void * typecast to vec3_t *
...
void CG_AddParticleToScene (cparticle_t *p, vec3_t org, float alpha)
{
	...
	else if (p->type == P_FLAT_SCALEUP)
	{
		...
		sinR = height * sin(DEG2RAD(p->roll)) * sqrt((float)2); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		cosR = width * cos(DEG2RAD(p->roll)) * sqrt((float)2); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
