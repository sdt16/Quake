...
void VM_LoadSymbols( vm_t *vm ) {
	...
	while ( 1 ) {
		...
		chars = strlen( token );
		sym = (vmSymbol_t *)Hunk_Alloc( sizeof( *sym ) + chars, h_high ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
		*prev = sym;
		prev = &sym->next;
		sym->next = NULL;
...
vm_t *VM_Create( const char *module, int (*systemCalls)(int *), 
	...
	vm->dataBase = (byte *)Hunk_Alloc( dataLength, h_high ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	...
	vm->instructionPointers = (int *)Hunk_Alloc( vm->instructionPointersLength, h_high ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
void VM_VmProfile_f( void ) {
	...
	sorted = (vmSymbol_t **)Z_Malloc( vm->numSymbols * sizeof( *sorted ) ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	sorted[0] = vm->symbols;
