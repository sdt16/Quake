...
static qboolean	CM_NeedsSubdivision( vec3_t a, vec3_t b, vec3_t c ) {
	...
	return((qboolean)(dist >= SUBDIVIDE_DISTANCE)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
	...
	for ( axis = 0 ; axis < 3 ; axis++ )
	{
		for ( dir = -1 ; dir <= 1 ; dir += 2, order++ )
		{
			...
			if ( i == facet->numBorders ) {
				...
				facet->borderNoAdjust[facet->numBorders] = (qboolean)0; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
static void CM_PatchCollideFromGrid( cGrid_t *grid, patchCollide_t *pf ) {
	...
	for ( i = 0 ; i < grid->width - 1 ; i++ ) {
		for ( j = 0 ; j < grid->height - 1 ; j++ ) {
			...
			if ( gridPlanes[i][j][0] == gridPlanes[i][j][1] ) {
				...
				facet->borderNoAdjust[0] = (qboolean)noAdjust[EN_TOP]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				facet->borderPlanes[1] = borders[EN_RIGHT];
				facet->borderNoAdjust[1] = (qboolean)noAdjust[EN_RIGHT]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				facet->borderPlanes[2] = borders[EN_BOTTOM];
				facet->borderNoAdjust[2] = (qboolean)noAdjust[EN_BOTTOM]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				facet->borderPlanes[3] = borders[EN_LEFT];
				facet->borderNoAdjust[3] = (qboolean)noAdjust[EN_LEFT]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				...
				facet->borderNoAdjust[0] = (qboolean)noAdjust[EN_BOTTOM]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
				facet->borderPlanes[1] = borders[EN_LEFT];
				facet->borderNoAdjust[1] = (qboolean)noAdjust[EN_LEFT]; // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	pf->facets = (facet_t *)Hunk_Alloc( numFacets * sizeof( *pf->facets ), h_high ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	Com_Memcpy( pf->facets, facets, numFacets * sizeof( *pf->facets ) );
	pf->planes = (patchPlane_t *)Hunk_Alloc( numPlanes * sizeof( *pf->planes ), h_high ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
struct patchCollide_s	*CM_GeneratePatchCollide( int width, int height, vec3_t *points ) {
	...
	// collided against
	pf = (patchCollide_t *)Hunk_Alloc( sizeof( *pf ), h_high ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
