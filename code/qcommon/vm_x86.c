...
// ***GREGS_VC9_PORT_MOD*** -- in our C++ build there's no _ftol() defined in the default libs, so I just put in a simple truncation function
...
void VM_Compile( vm_t *vm, vmHeader_t *header ) {
	...
	buf = (byte *)Z_Malloc( maxLength ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
	jused = (byte *)Z_Malloc(header->instructionCount + 2 ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
...
//			Emit4( vm->dataMask );
	...
	vm->codeBase = (byte *)Hunk_Alloc( compiledOfs, h_low ); // ***GREGS_VC9_PORT_MOD*** -- needed typecast
