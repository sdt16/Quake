...
qboolean FS_Initialized() {
	return (qboolean)(fs_searchpaths != NULL); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
static void FS_CopyFile( char *fromOSPath, char *toOSPath ) {
	...
	buf = (byte *)malloc( len ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
qboolean FS_FilenameCompare( const char *s1, const char *s2 ) {
	...
	do {
		...
		if (c1 != c2) {
			return((qboolean)-1);		// strings not equal // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
		}
	} while (c1);
	
	return((qboolean)0);		// strings are equal // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
char *FS_ShiftedStrStr(const char *string, const char *substring, int shift) {
	...
	buf[i] = '\0';
	return (char *)strstr(string, buf); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
}
...
int FS_ReadFile( const char *qpath, void **buffer ) {
	...
	if ( strstr( qpath, ".cfg" ) ) {
		...
		if ( com_journal && com_journal->integer == 2 ) {
			...
			buf = (byte *)Hunk_AllocateTempMemory(len+1); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
			*buffer = buf;
...
static pack_t *FS_LoadZipFile( char *zipfile, const char *basename )
{
	...
	buildBuffer = (fileInPack_t *)Z_Malloc( (gi.number_entry * sizeof( fileInPack_t )) + len ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	namePtr = ((char *) buildBuffer) + gi.number_entry * sizeof( fileInPack_t );
	fs_headerLongs = (int *)Z_Malloc( gi.number_entry * sizeof(int) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	...
	pack = (pack_t *)Z_Malloc( sizeof( pack_t ) + i * sizeof(fileInPack_t *) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	pack->hashSize = i;
...
char **FS_ListFilteredFiles( const char *path, const char *extension, char *filter, int *numfiles ) {
	...
	*numfiles = nfiles;
	...
	listCopy = (char **)Z_Malloc( ( nfiles + 1 ) * sizeof( *listCopy ) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	for ( i = 0 ; i < nfiles ; i++ ) {
		listCopy[i] = list[i];
	}
	listCopy[i] = NULL;
...
static char** Sys_ConcatenateFileLists( char **list0, char **list1, char **list2 )
{
  ...
  /* Create new list. */
  dst = cat = (char **)Z_Malloc( ( totalLength + 1 ) * sizeof( char* ) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
...
void FS_SortFileList(char **filelist, int numfiles) {
	...
	sortedlist = (char **)Z_Malloc( ( numfiles + 1 ) * sizeof( *sortedlist ) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	sortedlist[0] = NULL;
	numsortedfiles = 0;
...
static void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	//
	search = (searchpath_t *)Z_Malloc (sizeof(searchpath_t)); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	search->dir = (directory_t *)Z_Malloc( sizeof( *search->dir ) ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
