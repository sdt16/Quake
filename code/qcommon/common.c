...
void QDECL Com_Error( int code, const char *fmt, ... ) {
	...
	if ( code != ERR_DISCONNECT && code != ERR_NEED_CD ) {
		if (!com_noErrorInterrupt->integer) {
			__asm {
				//int 0x03 ***GREGS_VC9_PORT_MOD*** -- I want to SEE the exception messages!
			}
		}
	}
#endif
...
void *Z_TagMalloc( int size, int tag ) {
#endif
	int		extra, allocSize;
	memblock_t	*start, *rover, *new_block, *base;  // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
	memzone_t *zone;
	...
	if (extra > MINFRAGMENT) {
		...
		new_block = (memblock_t *) ((byte *)base + size ); // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->size = extra; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->tag = 0;			// free block // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->prev = base; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->id = ZONEID; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->next = base->next; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		new_block->next->prev = new_block; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		base->next = new_block; // ***GREGS_VC9_PORT_MOD*** -- variable "new" changed to "new_block" because "new" is a C++ keyword
		base->size = size;
	}
...
char *CopyString( const char *in ) {
	...
	out = (char *)S_Malloc (strlen(in)+1); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	strcpy (out, in);
	return out;
}
...
void Com_InitSmallZoneMemory( void ) {
	...
	// bk001205 - was malloc
	smallzone = (memzone_t *)calloc( s_smallZoneTotal, 1 ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	if ( !smallzone ) {
...
void Com_InitZoneMemory( void ) {
	...
	// bk001205 - was malloc
	mainzone = (memzone_t *)calloc( s_zoneTotal, 1 ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	if ( !mainzone ) {
...
void Com_InitHunkMemory( void ) {
	...
	// bk001205 - was malloc
	s_hunkData = (byte *)calloc( s_hunkTotal + 31, 1 ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	if ( !s_hunkData ) {
...
void Hunk_Trash( void ) {
	...
	if ( hunk_permanent == &hunk_low ) {
		buf = (char *)(s_hunkData + hunk_permanent->permanent); // ***GREGS_VC9_PORT_MOD*** -- changed typecast; was void *
	} else {
		buf = (char *)(s_hunkData + s_hunkTotal - hunk_permanent->permanent ); // ***GREGS_VC9_PORT_MOD*** -- changed typecast; was void *
	}
...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
        ...
        case SE_NONE:
            break;
		case SE_KEY:
			CL_KeyEvent( ev.evValue, (qboolean)ev.evValue2, ev.evTime ); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
			break;
		case SE_CHAR:
...
static void ConcatRemaining( const char *src, const char *start ) {
	...
	str = (char *)strstr(src, start); // ***GREGS_VC9_PORT_MOD*** -- added typecast(s)
	if (!str) {
		keyConcatArgs();
		return;
	}
